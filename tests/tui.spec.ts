...
describe("UI-neutral command with optional dsh-tui completion", () => {
  ...
  it("registers one provider command when dsh-tui is present", async () => {
    ...
    expect(
      commandTree.children(["codex", "hud"]).map((item) => item.name)
    ).toEqual(["on", "off", "toggle", "pin", "unpin", "status"]);
    expect(
    ...
    expect(
      commandTree
        .children(["codex", "set", "reasoning-summary"])
        .map((item) => item.name)
    ).toEqual(["auto", "concise", "detailed"]);
    await expect(
    ...
    expect(config.text).toContain("reasoning-summary: auto");
    expect(config.text).toContain("usage-hud: on (compact)");
    ...
    await expect(
      definition.handler({ rawInput: " set reasoning-summary detailed" } as never)
    ).resolves.toMatchObject({
      kind: "success",
    ...
    await expect(
      definition.handler({ rawInput: " hud off" } as never)
    ).resolves.toEqual({ kind: "success", text: "Codex usage HUD: off (compact)" });
    expect(service.updateUsageUiPreferences).toHaveBeenCalledWith({
      showUsageHud: false,
    });
    await expect(
      definition.handler({ rawInput: " hud toggle" } as never)
    ).resolves.toEqual({ kind: "success", text: "Codex usage HUD: on (compact)" });
    await expect(
      definition.handler({ rawInput: " hud pin" } as never)
    ).resolves.toEqual({ kind: "success", text: "Codex usage HUD: on (pinned)" });
    expect(service.updateUsageUiPreferences).toHaveBeenCalledWith({
      pinUsageHud: true,
    });
    await expect(
      definition.handler({ rawInput: " hud status" } as never)
    ).resolves.toEqual({ kind: "success", text: "Codex usage HUD: on (pinned)" });
