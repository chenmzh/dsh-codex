...
const CODEX_ACTIONS: readonly TuiSubcommandNode[] = [
  ...
  translatedNode(
    "hud",
    "Show, hide, or toggle the Codex usage HUD",
    "显示、隐藏或切换 Codex 用量条"
  ),
...
function codexSubcommands(
  ...
  if (path.length === 3 && path[0] === "codex" && path[1] === "set") {
    ...
    if (CODEX_SETTINGS.some((setting) => setting.name === path[2]))
  ...
  switch (key) {
    ...
    case "reasoning-summary":
      await service.updateResponsePreferences({
        reasoningSummary: value as OpenAICodexReasoningSummary,
      });
      return;
      ...
      try {
        switch (action) {
          ...
          case "hud": {
            ...
            if (
              parts.length > 2 ||
              !["on", "off", "toggle", "pin", "unpin", "status"].includes(requested)
            )
              return failure(HELP);
            const current = service.usageUiPreferences();
            if (requested === "pin" || requested === "unpin") {
              const next = await service.updateUsageUiPreferences({
                pinUsageHud: requested === "pin",
              });
              return success(
                "Codex usage HUD: " +
          ...
          case "set": {
            ...
            const valid =
              key === "reasoning-summary"
                ...
                : value === "on" || value === "off";
            if (parts.length !== 3 || key === undefined || value === undefined || !valid)
              return failure(HELP);
