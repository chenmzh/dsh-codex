...
export function OpenAICodexSettings({ t }: OpenAICodexSettingsProps) {
  ...
  const signIn = async (
    ...
    const popup =
      method === "browser" ? window.open("about:blank", "_blank") : null;
    ...
    try {
      ...
      if (challenge.method !== method)
        throw new Error("OpenAI Codex returned the wrong sign-in challenge");
          ...
          <select
            ...
            onChange={(event) => {
              void updateResponseApi({
                reasoningSummary:
                  event.currentTarget.value as ResponseApiPreferences["reasoningSummary"],
              });
            }}>
